        ...
        init(container, options) {
            ...
            function show_hide_filter_container() {
                $('.media-boxes-drop-down, .media-boxes-filter, .media-boxes-checkboxes').each(function () {
                    ...
                    if ($this.find('[data-filter]')[0] == undefined)
                        return; // only do this if there are filters
            ...
            function setupOverlayForHoverEffect(container) {
                if (container.data('settings').thumbnailOverlay == false)
                    return;
            ...
            $container.on('mouseenter.hoverdir, mouseleave.hoverdir', boxImageSelector, function (event) {
                if (settings.thumbnailOverlay == false)
                    return;
            ...
            var _getDir = function ($el, coordinates) {
                ...
                var w = $el.width(), h = $el.height(),
                    ...
                    x = (coordinates.x - $el.offset().left - (w / 2)) * (w > h ? (h / w) : 1), y = (coordinates.y - $el.offset().top - (h / 2)) * (h > w ? (w / h) : 1),
